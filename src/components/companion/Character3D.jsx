...
const Character3D=forwardRef(function Character3D({className="",characterId=DEFAULT_CHARACTER_ID,appearance={},wardrobe={},emotion="CALM",activity="idle",scene="sala",speaking=false,mouthIntensity=0,onCapabilities},ref){
 ...
 useEffect(()=>{
  ...
  const onLoad=asset=>{if(cancelled)return;model=asset.scene||asset;clips=asset.animations||model.animations||[];const b=new THREE.Box3().setFromObject(model),s=b.getSize(new THREE.Vector3());if(s.y>.01){model.scale.multiplyScalar(1.85/s.y);const nb=new THREE.Box3().setFromObject(model),c=nb.getCenter(new THREE.Vector3());model.position.x-=c.x;model.position.z-=c.z;model.position.y-=nb.min.y;}
 ...
 useEffect(()=>apiRef.current?.applyTints(),[appearance]);useEffect(()=>apiRef.current?.applyWardrobe(),[wardrobe]);useEffect(()=>apiRef.current?.setEmotion(),[emotion]);useEffect(()=>apiRef.current?.applyActivity(),[activity]);useEffect(()=>{ /* scene handled by renderer lifecycle */ },[scene]);
